...
def gramm_Hulk_LR1():
    ...
    compAritCond, compBoolCond, base_args, let_in, simple_assignment, multi_assignment, kern_assignment  = G.NonTerminals('compAritCond compBoolCond base_args let_in simple_assignment multi_assignment kern_assignment')
    ...
    condition %= expression + compAritCond + expression | Not + condition | condition + compBoolCond + condition | _True | _False | expression + Is + def_Type | oPar + condition + cPar | function_call | identifier
    compAritCond %= Less | Greater | Equal | LessEqual | GreaterEqual | NotEqual 
    compBoolCond %= And | Or 
    expression %= term + Plus + term | term + Minus + term | term 
    term %= factor + Mult + factor | factor + Div + factor | factor + Mod + factor | factor
    ...
    attribute_definition %= identifier + type_annotation + Equal + expression + Semi + attribute_definition | G.Epsilon
    method_definition %= identifier + oPar + parameters + cPar + oBrace + expression + cBrace + method_definition | G.Epsilon
    ...
    method_override %= identifier + oPar + parameters + cPar + oBrace + expression + Semi + cBrace + method_override | G.Epsilon
